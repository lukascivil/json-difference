...
server.registerTool('get_diff', {
    title: 'Get JSON diff',
    description: 'Returns the delta between two JSON structures: { added, removed, edited }. ' +
    ...
    inputSchema: {
        ...
        isLodashLike: z
            .boolean()
            .optional()
            .describe('When true, paths use lodash-style bracket notation (e.g. "a[0].b")')
    }
}, async ({ original, modified, isLodashLike }) => {
    const delta = getDiff(parseJson(original), parseJson(modified), {
...
server.registerTool('get_edited_paths', {
...
server.registerTool('get_struct_paths', {
...
server.registerTool('get_paths_diff', {
